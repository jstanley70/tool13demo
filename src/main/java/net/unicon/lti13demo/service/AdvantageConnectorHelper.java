...
public class AdvantageConnectorHelper {
 ...
 // We put the token in the Authorization as a simple Bearer one.
    public HttpEntity createTokenizedRequestEntity(Token token, Map<String,String> map) {
        HttpHeaders headers = new HttpHeaders();
        headers.set(HttpHeaders.AUTHORIZATION, "Bearer " + token.getAccess_token());
        ...
        try {
            ...
            if (reportPostResponse != null) {
                ...
                if (status.is2xxSuccessful()) {
                    ...
                    getTokenWithEncodedForm(platformDeployment, scope);
                }
            } else {
                log.warn("Problem getting the token");
                return getTokenWithEncodedForm(platformDeployment, scope);
            }
        } catch (Exception e) {
    ...
    public Token getTokenWithEncodedForm(PlatformDeployment platformDeployment, String scope) throws ConnectionException {
        Token token = null;
        try {
            RestTemplate restTemplate = createFormRestTemplate();
            // We need an specific request for the token.
            HttpEntity request = createTokenRequestEncodedForm(scope, platformDeployment);
            final String POST_TOKEN_URL = platformDeployment.getoAuth2TokenUrl();
            log.debug("POST_TOKEN_URL -  "+ POST_TOKEN_URL);
            ResponseEntity<Token> reportPostResponse = restTemplate.
                    postForEntity(POST_TOKEN_URL, request, Token.class);
            ...
            try {
				...
				return mapper.readValue(e.getMessage(), Token.class);
 ...
 // This is specific to request a token.
    public HttpEntity createTokenRequestEncodedForm(String scope, PlatformDeployment platformDeployment) throws GeneralSecurityException, IOException {
        HttpHeaders headers = new HttpHeaders();
        ...
        String jwtToken = ltijwtService.generateTokenRequestJWT(platformDeployment);
        log.debug("jwt string createTokenRequest:", jwtToken);
        form.add("client_assertion", jwtToken);
        //We need to pass the scope of the token, meaning, the service we want to allow with this token.
    ...
    public RestTemplate createRestTemplate() {
        ...
        addJacksonConverter(restTemplate, messageConverters);
        addErrorConverter(restTemplate, messageConverters);
    ...
    public RestTemplate createFormRestTemplate() {
        RestTemplate restTemplate = new RestTemplate(
                new BufferingClientHttpRequestFactory(new SimpleClientHttpRequestFactory()));
        List<HttpMessageConverter<?>> messageConverters = new ArrayList<>();
        encodedFormConverter(restTemplate, messageConverters);
        addJacksonConverter(restTemplate, messageConverters);
        addErrorConverter(restTemplate, messageConverters);
        restTemplate.setMessageConverters(messageConverters);
        return restTemplate;
    ...
    private void addJacksonConverter(RestTemplate restTemplate, List<HttpMessageConverter<?>> messageConverters) {
        MappingJackson2HttpMessageConverter converter = new MappingJackson2HttpMessageConverter();
        List<MediaType> supportedMediaTypes = new ArrayList<>();
        supportedMediaTypes.add(MediaType.APPLICATION_JSON);
        supportedMediaTypes.addAll(AgsTokens.getAgsMediaTypes());
        supportedMediaTypes.addAll(MembershipTokens.getMembershipMediaTypes());
        converter.setSupportedMediaTypes(supportedMediaTypes);
        messageConverters.add(converter);
    }
    
    private void addErrorConverter(RestTemplate restTemplate, List<HttpMessageConverter<?>> messageConverters) {
         messageConverters.add(new MessageConverterHtml());
         restTemplate.setErrorHandler(new RestTemplateResponseErrorHandler());
    }
    
    private void encodedFormConverter(RestTemplate restTemplate, List<HttpMessageConverter<?>> messageConverters) {
        List<MediaType> supportedMediaTypes = new ArrayList<>();
        supportedMediaTypes.add(MediaType.APPLICATION_FORM_URLENCODED);
        FormHttpMessageConverter formConverter = new FormHttpMessageConverter();
        formConverter.setSupportedMediaTypes(supportedMediaTypes);
